...
def define_generator_variables_constraints(network, snapshots):
    ...
    fixed_committable_gens_i = network.generators.index[
        ~network.generators.p_nom_extendable & network.generators.committable
    ]

    if (network.generators.p_nom_extendable & network.generators.committable).any():
        logger.warning(
            "The following generators have both investment optimisation and unit commitment:\n{}\nCurrently PyPSA cannot do both these functions, so PyPSA is choosing investment optimisation for these generators.".format(
    ...
    bad_uc_gens = network.generators.index[
    ...
    if not bad_uc_gens.empty:
        logger.warning(
            "The following committable generators were both up and down before the simulation: {}. This will cause an infeasibility.".format(
                bad_uc_gens
            )
        )
    ...
    if len(fixed_gens_i):
        var_lower = p_min_pu.loc[:, fixed_gens_i].multiply(
            network.generators.loc[fixed_gens_i, "p_nom"]
        )
        var_upper = p_max_pu.loc[:, fixed_gens_i].multiply(
            network.generators.loc[fixed_gens_i, "p_nom"]
        )

        gen_p_bounds.update(
            {
                (gen, sn): (var_lower[gen][sn], var_upper[gen][sn])
    ...
    network.model.generator_p = Var(
        list(network.generators.index), snapshots, domain=Reals, bounds=gen_p_bounds_f
    )
    ...
    network.model.generator_p_nom = Var(
        list(extendable_gens_i), domain=NonNegativeReals, bounds=gen_p_nom_bounds
    )
    ...
    network.model.generator_status = Var(
        list(fixed_committable_gens_i), snapshots, within=Binary
    )

    var_lower = p_min_pu.loc[:, fixed_committable_gens_i].multiply(
        network.generators.loc[fixed_committable_gens_i, "p_nom"]
    )
    var_upper = p_max_pu.loc[:, fixed_committable_gens_i].multiply(
        network.generators.loc[fixed_committable_gens_i, "p_nom"]
    )
    ...
    committable_gen_p_lower = {
        (gen, sn): LConstraint(
            LExpression(
                [
                    (var_lower[gen][sn], network.model.generator_status[gen, sn]),
                    (-1.0, network.model.generator_p[gen, sn]),
                ]
            ),
            "<=",
        )
    ...
    committable_gen_p_upper = {
        (gen, sn): LConstraint(
            LExpression(
                [
                    (var_upper[gen][sn], network.model.generator_status[gen, sn]),
                    (-1.0, network.model.generator_p[gen, sn]),
                ]
            ),
            ">=",
    ...
    up_time_gens = fixed_committable_gens_i[
    ...
    for gen_i, gen in enumerate(up_time_gens):
        ...
        if up_time_before == start_i:
            up_time_before = min(
                min_up_time, start_i + network.generators.at[gen, "up_time_before"]
            )
    ...
    down_time_gens = fixed_committable_gens_i[
        network.generators.loc[fixed_committable_gens_i, "min_down_time"] > 0
    ]
    ...
    for gen_i, gen in enumerate(down_time_gens):
        ...
        if down_time_before == start_i:
            down_time_before = min(
                min_down_time, start_i + network.generators.at[gen, "down_time_before"]
            )
    ...
    suc_gens = fixed_committable_gens_i[
        network.generators.loc[fixed_committable_gens_i, "start_up_cost"] > 0
    ]

    network.model.generator_start_up_cost = Var(
        list(suc_gens), snapshots, domain=NonNegativeReals
    )
    ...
    for gen in suc_gens:
        ...
        if start_i == 0:
            ...
            initial_status = network.generators_t.status.at[
                network.snapshots[start_i - 1], gen
            ]
    ...
    sdc_gens = fixed_committable_gens_i[
        network.generators.loc[fixed_committable_gens_i, "shut_down_cost"] > 0
    ]

    network.model.generator_shut_down_cost = Var(
        list(sdc_gens), snapshots, domain=NonNegativeReals
    )
            ...
            if network.generators.at[gen, "p_nom_extendable"]:
                ...
                lhs = LExpression(
                    [
                        ...
                        (
                            (
                                network.generators.at[gen, "ramp_limit_start_up"]
                                - network.generators.at[gen, "ramp_limit_up"]
                            )
                            * network.generators.at[gen, "p_nom"],
    ...
    if start_i > 0:
        ...
        for gen in ru_gens:
            ...
            if network.generators.at[gen, "p_nom_extendable"]:
                ...
                status_prev = network.generators_t.status.at[
                    network.snapshots[start_i - 1], gen
                ]
                lhs = LExpression(
                    [
        ...
        for sn, sn_prev in zip(snapshots[1:], snapshots[:-1]):
            if network.generators.at[gen, "p_nom_extendable"]:
                ...
                lhs = LExpression(
                    ...
                    network.generators.loc[gen, "ramp_limit_down"]
                    * network.generators.at[gen, "p_nom"],
                )
            else:
                lhs = LExpression(
                    [
                        ...
                        (
                            (
                                network.generators.at[gen, "ramp_limit_down"]
                                - network.generators.at[gen, "ramp_limit_shut_down"]
                            )
                            * network.generators.at[gen, "p_nom"],
                            network.model.generator_status[gen, sn],
                        ),
...
def define_storage_variables_constraints(network, snapshots):
    ...
    bounds.update(
        {
            (su, sn): (0, sus.at[su, "p_nom"] * p_max_pu.at[sn, su])
            for su in fix_sus_i
            for sn in snapshots
        }
    )
    ...
    network.model.storage_p_dispatch = Var(
        list(network.storage_units.index),
        snapshots,
        domain=NonNegativeReals,
        bounds=su_p_dispatch_bounds,
    )
    ...
    bounds.update(
        {
            (su, sn): (0, -sus.at[su, "p_nom"] * p_min_pu.at[sn, su])
            for su in fix_sus_i
            for sn in snapshots
        }
    )
    ...
    network.model.storage_p_store = Var(
        list(network.storage_units.index),
        snapshots,
        domain=NonNegativeReals,
        bounds=su_p_store_bounds,
    )
    ...
    network.model.storage_p_spill = Var(
        list(spill_index), domain=NonNegativeReals, bounds=su_p_spill_bounds
    )
    ...
    def su_p_nom_bounds(model, su_name):
        return (sus.at[su_name, "p_nom_min"], sus.at[su_name, "p_nom_max"])

    network.model.storage_p_nom = Var(
        list(ext_sus_i), domain=NonNegativeReals, bounds=su_p_nom_bounds
    )
    ...
    upper = {
        (su, sn): [
            [
                (1, model.state_of_charge[su, sn]),
                (-sus.at[su, "max_hours"], model.storage_p_nom[su]),
            ],
            "<=",
            0.0,
        ]
...
def define_store_variables_constraints(network, snapshots):
    ...
    bounds.update(
        {
            (store, sn): (
                stores.at[store, "e_nom"] * e_min_pu.at[sn, store],
                stores.at[store, "e_nom"] * e_max_pu.at[sn, store],
            )
    ...
    network.model.store_e = Var(
        list(stores.index), snapshots, domain=Reals, bounds=store_e_bounds
    )
    ...
    def store_e_nom_bounds(model, store):
        return (stores.at[store, "e_nom_min"], stores.at[store, "e_nom_max"])

    network.model.store_e_nom = Var(
        list(ext_stores), domain=Reals, bounds=store_e_nom_bounds
    )
    ...
    def store_e_upper(model, store, snapshot):
        return (
            model.store_e[store, snapshot]
            <= model.store_e_nom[store] * e_max_pu.at[snapshot, store]
        )

    network.model.store_e_upper = Constraint(
        list(ext_stores), snapshots, rule=store_e_upper
    )
    ...
    network.model.store_e_lower = Constraint(
        list(ext_stores), snapshots, rule=store_e_lower
    )
    ...
    for store in stores.index:
        for i, sn in enumerate(snapshots):
            ...
            e[store, sn].lhs.variables.append(
                (-elapsed_hours, model.store_p[store, sn])
            )
...
def define_branch_extension_variables(network, snapshots):
    ...
    bounds = {
        b: (
            extendable_passive_branches.at[b, "s_nom_min"],
    ...
    network.model.passive_branch_s_nom = Var(
        list(extendable_passive_branches.index),
        domain=NonNegativeReals,
        bounds=branch_s_nom_bounds,
    )
    ...
    bounds = {
        b: (extendable_links.at[b, "p_nom_min"], extendable_links.at[b, "p_nom_max"])
    ...
    network.model.link_p_nom = Var(
        list(extendable_links.index),
        domain=NonNegativeReals,
        bounds=branch_p_nom_bounds,
    )
...
def define_link_flows(network, snapshots):
    ...
    fixed_upper = p_max_pu.loc[:, fixed_links_i].multiply(
        network.links.loc[fixed_links_i, "p_nom"]
    )
    ...
    p_upper = {
        (cb, sn): LConstraint(
            LExpression([(1, network.model.link_p[cb, sn])], -fixed_upper.at[sn, cb]),
            "<=",
        )
        for cb in fixed_links_i
        for sn in snapshots
    }
    ...
    p_upper.update(
        {
            (cb, sn): LConstraint(
                LExpression(
                    [
                        (1, network.model.link_p[cb, sn]),
                        (-p_max_pu.at[sn, cb], network.model.link_p_nom[cb]),
                    ]
                ),
                "<=",
            )
            for cb in extendable_links_i
            for sn in snapshots
        }
    )

    l_constraint(
        network.model, "link_p_upper", p_upper, list(network.links.index), snapshots
    )

    p_lower = {
        (cb, sn): LConstraint(
            LExpression([(1, network.model.link_p[cb, sn])], -fixed_lower.at[sn, cb]),
            ">=",
        )
        for cb in fixed_links_i
        for sn in snapshots
    ...
    l_constraint(
        network.model, "link_p_lower", p_lower, list(network.links.index), snapshots
    )
...
def define_passive_branch_flows_with_angles(network, snapshots):
    ...
    slack = {
        (sub, sn): [
            [
                (
                    1,
                    network.model.voltage_angles[
                        network.sub_networks.slack_bus[sub], sn
                    ],
                )
            ],
            "==",
    ...
    l_constraint(
        network.model, "slack_angle", slack, list(network.sub_networks.index), snapshots
    )
    ...
    for branch in passive_branches.index:
        ...
        attribute = (
            "r_pu_eff"
            if network.sub_networks.at[sub, "carrier"] == "DC"
            else "x_pu_eff"
        )
        ...
        for sn in snapshots:
            lhs = LExpression(
                [
                    (y, network.model.voltage_angles[bus0, sn]),
                    (-y, network.model.voltage_angles[bus1, sn]),
                    (-1, network.model.passive_branch_p[bt, bn, sn]),
                ],
                -y
                * (
                    passive_branches.at[branch, "phase_shift"] * np.pi / 180.0
                    if bt == "Transformer"
                    else 0.0
                ),
            )
...
def define_sub_network_cycle_constraints(
    subnetwork, snapshots, passive_branch_p, attribute
):
    """
    ...
    for col_j in range(matrix.shape[1]):
        ...
        for cycle_i in cycle_is:
            ...
            attribute_value = (
                1e5 * branches.at[branch_idx, attribute] * subnetwork.C[cycle_i, col_j]
            )
...
def define_passive_branch_flows_with_cycles(network, snapshots):
    ...
    for subnetwork in network.sub_networks.obj:
        ...
        (
            ...
            subnetwork, snapshots, network.model.passive_branch_p, attribute
        )
    ...
    l_constraint(
        network.model, "cycle_constraints", cycle_constraints, cycle_index, snapshots
    )

    network.model.cycles = Var(
        cycle_index, snapshots, domain=Reals, bounds=(None, None)
    )
    ...
    for subnetwork in network.sub_networks.obj:
        ...
        for i, branch in enumerate(branches.index):
            ...
            if len(cycle_is) + len(tree_is) == 0:
                logger.error(
                    "The cycle formulation does not support infinite impedances, yet."
                )
            ...
            for snapshot in snapshots:
                expr = LExpression(
                    [
                        (
                            subnetwork.C[i, j],
                            network.model.cycles[subnetwork.name, j, snapshot],
                        )
                        for j in cycle_is
                    ]
                )
                lhs = expr + sum(
                    subnetwork.T[i, j] * network._p_balance[buses.index[j], snapshot]
                    for j in tree_is
                )

                rhs = LExpression(
                    [(1, network.model.passive_branch_p[bt, bn, snapshot])]
                )
...
def define_nodal_balances(network, snapshots):
    ...
    if links.size > 0:
        logger.warning(
            "Some bidirectional links have efficiency values lower than 1: '"
            + "', '".join(links)
            + "'."
        )
    ...
    for su in network.storage_units.index:
        ...
        #Luca: here is issue
            ...
            network._p_balance[bus, sn].variables.append(
                (sign, network.model.storage_p_dispatch[su, sn])
            )
            network._p_balance[bus, sn].variables.append(
                (-sign, network.model.storage_p_store[su, sn])
            )
    ...
    for store in network.stores.index:
        ...
        for sn in snapshots:
            network._p_balance[bus, sn].variables.append(
                (sign, network.model.store_p[store, sn])
            )
    ...
    for sub_network in network.sub_networks.obj:
        for sn in snapshots:
            ...
            for bus in sub_network.buses().index:
                sn_balance[sub_network.name, sn].lhs.variables.extend(
                    network._p_balance[bus, sn].variables
                )
                sn_balance[sub_network.name, sn].lhs.constant += network._p_balance[
                    bus, sn
                ].constant

    l_constraint(
        network.model,
        "sub_network_balance_constraint",
        sn_balance,
        list(network.sub_networks.index),
        snapshots,
    )
...
def define_global_constraints(network, snapshots):
    ...
    for gc in network.global_constraints.index:
        if network.global_constraints.loc[gc, "type"] == "primary_energy":
            ...
            for carrier in network.carriers.index:
                ...
                sus = network.storage_units.index[
                    (network.storage_units.carrier == carrier)
                    & (~network.storage_units.cyclic_state_of_charge)
                ]
                c.lhs.variables.extend(
                    [
                        (-attribute, network.model.state_of_charge[su, snapshots[-1]])
                        for su in sus
                    ]
                )
                c.lhs.constant += sum(
                    attribute * network.storage_units.at[su, "state_of_charge_initial"]
                    for su in sus
                )
                ...
                stores = network.stores.index[
                    (network.stores.bus.map(network.buses.carrier) == carrier)
    ...
    l_constraint(
        ...
        "global_constraints",
        global_constraints,
        list(network.global_constraints.index),
    )
...
def define_linear_objective(network, snapshots):
    ...
    suc_gens_i = network.generators.index[
        ~network.generators.p_nom_extendable
        & network.generators.committable
        & (network.generators.start_up_cost > 0)
    ]

    sdc_gens_i = network.generators.index[
        ~network.generators.p_nom_extendable
        & network.generators.committable
        & (network.generators.shut_down_cost > 0)
    ]

    marginal_cost_it = zip(
        get_switchable_as_iter(network, "Generator", "marginal_cost", snapshots),
    ...
    for sn, marginal_cost in zip(snapshots, marginal_cost_it):
        ...
        for su in network.storage_units.index:
            ...
            objective.variables.extend(
                [(coefficient, model.storage_p_dispatch[su, sn])]
            )
    ...
    objective.variables.extend(
        [
            (extendable_generators.at[gen, "capital_cost"], model.generator_p_nom[gen])
            for gen in extendable_generators.index
        ]
    )
    objective.constant -= (
        extendable_generators.capital_cost * extendable_generators.p_nom
    ).zsum()

    objective.variables.extend(
        [
            (ext_sus.at[su, "capital_cost"], model.storage_p_nom[su])
    ...
    objective.variables.extend(
        [
            (ext_stores.at[store, "capital_cost"], model.store_e_nom[store])
    ...
    objective.variables.extend(
        [
            (
                extendable_passive_branches.at[b, "capital_cost"],
                model.passive_branch_s_nom[b],
            )
            for b in extendable_passive_branches.index
        ]
    )
    objective.constant -= (
        extendable_passive_branches.capital_cost * extendable_passive_branches.s_nom
    ).zsum()

    objective.variables.extend(
        [
            (extendable_links.at[b, "capital_cost"], model.link_p_nom[b])
            for b in extendable_links.index
        ]
    )
    objective.constant -= (
        extendable_links.capital_cost * extendable_links.p_nom
    ).zsum()
...
def extract_optimisation_results(
    ...
    duals = pd.Series(
        list(model.dual.values()), index=pd.Index(list(model.dual.keys())), dtype=float
    )
    ...
    if len(network.storage_units):
        set_from_series(
            ...
            get_values(model.storage_p_dispatch) - get_values(model.storage_p_store),
        )

        set_from_series(
            network.storage_units_t.state_of_charge, get_values(model.state_of_charge)
        )

        if (network.storage_units_t.inflow.max() > 0).any():
            set_from_series(
                network.storage_units_t.spill, get_values(model.storage_p_spill)
            )
        network.storage_units_t.spill.fillna(
            0, inplace=True
        )  # p_spill doesn't exist if inflow=0
    ...
    if len(network.buses):
        if formulation in {"angles", "kirchhoff"}:
            set_from_series(
                ...
                pd.Series(
                    list(model.power_balance.values()),
        ...
        if formulation == "angles":
            set_from_series(network.buses_t.v_ang, get_values(model.voltage_angles))
        elif formulation in ["ptdf", "cycles", "kirchhoff"]:
            for sn in network.sub_networks.obj:
                network.buses_t.v_ang.loc[snapshots, sn.slack_bus] = 0.0
                if len(sn.pvpqs) > 0:
                    network.buses_t.v_ang.loc[snapshots, sn.pvpqs] = spsolve(
                        sn.B[1:, 1:], network.buses_t.p.loc[snapshots, sn.pvpqs].T
                    ).T

        network.buses_t.v_mag_pu.loc[snapshots, network.buses.carrier == "AC"] = 1.0
        network.buses_t.v_mag_pu.loc[snapshots, network.buses.carrier == "DC"] = (
            1 + network.buses_t.v_ang.loc[snapshots, network.buses.carrier == "DC"]
        )

    # now that we've used the angles to calculate the flow, set the DC ones to zero
    network.buses_t.v_ang.loc[snapshots, network.buses.carrier == "DC"] = 0.0
    ...
    network.stores.loc[network.stores.e_nom_extendable, "e_nom_opt"] = get_values(
    ...
    network.links.loc[network.links.p_nom_extendable, "p_nom_opt"] = get_values(
        network.model.link_p_nom
    )

    try:
        network.global_constraints.loc[:, "mu"] = -get_shadows(
            model.global_constraints, multiind=False
        )
...
def network_lopf_build_model(
    ...
    if isinstance(network.snapshots, pd.MultiIndex):
        raise NotImplementedError(
            "Optimization with multiindexed snapshots " "using pyomo is not supported."
        )
...
def network_lopf(
    ...
    if type(network.snapshots) == pd.MultiIndex:
        raise NotImplementedError(
            "Multi indexed snapshots is only supported for pyomo=False"
        )
    if network.links[["ramp_limit_up", "ramp_limit_down"]].notnull().any().any():
        logger.warning(
            "Encountered nonzero ramp limits for links. These are ignored when running the optimization with `pyomo=True`."
        )
